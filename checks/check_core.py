...
from SmartDrawer.core import (  # noqa: E402
    Candidate,
    Crop,
    Detection,
    DrawerState,
...
from SmartDrawer.semantic import SemanticCatalog  # noqa: E402
from SmartDrawer.simulator import filled, mask_filled, run_demo, with_patch  # noqa: E402
from SmartDrawer.storage import Storage  # noqa: E402
...
def check_simulation_semantics_and_models() -> None:
    ...
    catalog = SemanticCatalog.load(ROOT / "SmartDrawer/config/semantic_catalog.json")
    ...
    enabled = json.loads((ROOT / "SmartDrawer/config/enabled_classes.json").read_text(encoding="utf-8"))
    assert len(enabled["classes"]) == 20
    assert len((ROOT / "SmartDrawer/models/coco_labels.txt").read_text(encoding="utf-8").splitlines()) == 80
    for filename, expected in {
        ...
        digest = hashlib.sha256((ROOT / "SmartDrawer/models" / filename).read_bytes()).hexdigest()
