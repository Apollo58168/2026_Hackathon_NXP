...
```text
smart_drawer/             # Python package、硬體 runtime 與 probes
config/                   # runtime 設定與 semantic catalogs
models/                   # 模型、labels 與 tokenizer
assets/audio/             # 手動測試音訊
docs/DESIGN.md            # 系統設計
tests/test_core.py        # dependency-free acceptance checks
```
...
`tests/test_core.py` 使用 Python `assert`，至少驗證：
