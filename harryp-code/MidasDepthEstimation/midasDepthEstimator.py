...
class midasDepthEstimator():
	...
	def initializeModel(self):
		...
		modelPath = projectRoot / 'models' / 'midasModel.tflite'

		# Download the official TF Hub TFLite model if it is not available yet.
		if not modelPath.is_file():
			...
			print(f"Downloading MiDaS model to {modelPath} ...")
			try:
				urllib.request.urlretrieve(url, modelPath)
			except Exception as exc:
				raise RuntimeError(
					"Could not download the MiDaS TFLite model. Check your network connection "
					f"and retry. Source: {url}"
				) from exc

		self.interpreter = Interpreter(model_path=str(modelPath))
	...
	def processRawDisparity(self, rawDisparity, img_shape):
		...
		if depth_range <= np.finfo(np.float32).eps:
			normalizedDisparity = np.zeros_like(rawDisparity, dtype=np.uint8)
		else:
			normalizedDisparity = (255 * (rawDisparity - depth_min) / depth_range).astype("uint8")
